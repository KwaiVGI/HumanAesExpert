...
# copy from https://github.com/chaofengc/IQA-PyTorch/blob/main/pyiqa/metrics/correlation_coefficient.py
...
def calculate_rmae(x, y, fit_scale=None, eps=1e-8):
    if fit_scale is not None:
        x = fit_curve(x, y, fit_scale)
    return np.sqrt(np.mean(np.abs(x - y)) + eps)
