...
def trainer_train(
    ...
    if is_master():
        try:
            model = trainer.model.get_base_model()
            message = 'regression unsaved'
            message2 = 'full unsaved'
            message3 = 'regression2 unsaved'
            try:
                print(model.language_model.lm_regression_head)
                torch.save(model.language_model.lm_regression_head.state_dict(), os.path.join(args.output_dir, 'regression_head.pth'))
                message = 'regression saved'
            except Exception as e:
                message = str(e)

            try:
                print(model.language_model.expert_head)
                torch.save(model.language_model.expert_head.state_dict(), os.path.join(args.output_dir, 'expert_head.pth'))
                message3 = 'regression2 saved'
            except Exception as e:
                message3 = str(e)


            with open(os.path.join(args.output_dir, 'save.json'), 'w', encoding='utf-8') as f:
                message = {'message': message, 'message2' : message2, 'message3':message3}
                json.dump(message, f)
        except:
            print('it may be full sft')
