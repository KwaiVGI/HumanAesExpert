...
class Template:
    ...
    def data_collator(self, batch: List[Dict[str, Any]], padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        scores_labels = [b['scores_labels'] for b in batch if b.get('scores_labels') is not None]
        is_expert = [b['is_expert'] for b in batch if b.get('is_expert') is not None]

        if len(scores_labels) != 0:
            res['scores_labels'] = torch.stack(scores_labels)
        if len(is_expert) != 0:
            res['is_expert'] = torch.stack(is_expert)
