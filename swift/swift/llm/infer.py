...
def merge_lora(args: InferArguments,
    ...
    if os.path.exists(merged_lora_path) and not replace_if_exists:
        ...
        print('\n\n\ntrying load regression head\n\n\n')

        try:
            
            print(model.language_model.lm_regression_head.weight)
            temp_weights = torch.load(os.path.join(ckpt_dir, 'regression_head.pth'))
            print(temp_weights['weight'].cpu() ,temp_weights['weight'].device)
            model.language_model.lm_regression_head.load_state_dict(temp_weights)
            print('regression_head loaded')
        except Exception as e:
            print(e)

        print('\n\n\ntrying load expert head again\n\n\n')

        try:
            model.language_model.expert_head.load_state_dict(torch.load(os.path.join(ckpt_dir, 'expert_head.pth')))
            print('expert_head loaded')
        except Exception as e:
            print(e)    
